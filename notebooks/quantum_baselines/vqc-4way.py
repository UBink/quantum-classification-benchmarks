# ============================================================================
# VQC Experiments - 4-Way Multiclass Classification
# Mirrors binary vqc-500.py structure; adapted for 4-class MNIST (0,1,2,3)
# Datasets: mnist_multi4_pca_4 (4 qubits), mnist_multi4_pca_8 (8 qubits)
# Sample sizes: 100, 250, 400 (matching quantum_qsvc_4way)
# ============================================================================
...
DATASETS = [
    "mnist_multi4_pca_4",   # 4 features -> 4 qubits
    "mnist_multi4_pca_8",   # 8 features -> 8 qubits
]

SAMPLE_SIZES = [100, 250, 400]          # Match QSVC 4-way sample sizes
SEEDS = [42, 100, 20, 5, 99]           # Same 5 seeds throughout

FEATURE_MAP_REPS = 1
ANSATZ_REPS = 1                         # Matches binary vqc_results.json (ansatz_reps=1)
ENTANGLEMENT = 'linear'
MAX_ITER = 100

DATA_PATH = Path("data/multiclass/processed")
RESULTS_PATH = Path("results/vqc_4way_results.json")
...
def create_vqc(n_features, feature_map_reps=1, ansatz_reps=1, max_iter=100):
    """Create Variational Quantum Classifier for multiclass."""
...
def run_single_experiment(dataset_name, n_samples, seed,
                          feature_map_reps=1, ansatz_reps=1, max_iter=100):
    """Run a single 4-way VQC experiment."""
    ...
    print(f"  Dataset: {dataset_name} | n_train={n_samples} | "
          f"n_features={n_features} | seed={seed}")
...
def run_all_experiments(resume=True):
    """Run all 4-way VQC experiments with resume support."""

    print("\n" + "=" * 70)
    print("VARIATIONAL QUANTUM CLASSIFIER - 4-WAY MULTICLASS")
    print("Classes: 0, 1, 2, 3  |  Datasets: pca_4 and pca_8")
    print("=" * 70)
    ...
    if RESULTS_PATH.exists() and resume:
        ...
        all_results = {
            "experiment_info": {
                "model_type":      "variational_quantum_classifier_4way",
        ...
        for n_samples in SAMPLE_SIZES:
            ...
            for seed in SEEDS:
                ...
                try:
                    ...
                    print(f"  ✓ Train time: {result['training_time_seconds']:.1f}s")
    ...
    print("\n" + "=" * 70)
    print("4-WAY VQC EXPERIMENTS COMPLETE")
    print("=" * 70)
