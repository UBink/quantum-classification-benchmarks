# ============================================================================
# QSVC Quantum Baseline Experiments - 4-Way Multiclass (digits 0,1,2,3)
# ============================================================================
...
DATASETS = [
    "mnist_multi4_pca_4",   # 4 qubits
    "mnist_multi4_pca_8",   # 8 qubits
]
...
RESULTS_PATH = Path("results/quantum_qsvc_4way_results.json")
...
def load_and_prepare_data(dataset_name, n_samples=None, seed=42):
    ...
    assert len(unique_labels) == 4, \
        f"Expected 4 classes, got {len(unique_labels)}: {unique_labels}"
...
def create_feature_map(n_features, reps=1, entanglement='linear'):
    """Create ZZ feature map for n_features qubits."""
...
def run_single_experiment(
    ...
    """
    Run a single 4-way QSVC experiment.

    QSVC internally uses sklearn's SVC which handles multiclass via
    one-vs-one (OvO) decomposition — no extra changes needed for 4-class.
    """
    ...
    # Training
    print(f"  Training QSVC (4-way OvO)...")
...
def run_all_experiments(resume=True):
    """Run all 4-way QSVC experiments with resumability."""
    ...
    if RESULTS_PATH.exists() and resume:
        ...
        all_results = {
            "experiment_info": {
                "model_type": "quantum_kernel_svm_4way",
                ...
                "multiclass_strategy": "one_vs_one",
                "datasets": DATASETS,
                "qubit_configs": {"mnist_multi4_pca_4": 4, "mnist_multi4_pca_8": 8},
            },
            "results": []
        }
